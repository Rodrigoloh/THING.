...
export function ThingNav({ thingId }: { thingId: string }) {
  ...
  return (
    <nav aria-label={t.navAreas} className="grid grid-cols-4 gap-1 rounded-[18px] border border-border bg-surface p-1">
      {[ [t.navHome, ""], [t.space, "/space"], [t.chat, "/chat"], [t.moments, "/moments"] ].map(([label, suffix]) => (
