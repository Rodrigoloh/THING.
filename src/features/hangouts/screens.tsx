...
import {
  ...
  skipHotPrompt, startChoiceEngine, startHot, startSameBrain, submitChoiceAnswer, submitHotEscalation, submitHotRound, submitSameBrainAnswer,
} from './actions';
...
function ChoiceGame({ initial }: { initial: ChoiceSnapshot }) {
  ...
  const [snapshot, setSnapshot] = useState(initial); const [error, setError] = useState<FlowError | null>(null); const [pending, transition] = useTransition(); const round = snapshot.round;
  ...
  if (round.state === 'revealed') { const subject = round.answers.find((a) => a.is_subject); const predictor = round.answers.find((a) => !a.is_subject); const agreed = game === 'know_me' ? subject?.answer_key === predictor?.answer_key : round.answers[0]?.answer_key === round.answers[1]?.answer_key; return <div className="space-y-5 text-center"><RoundLabel number={round.number} /><p className="text-xl">{prompt}</p><p className={`font-heading text-5xl font-black ${agreed ? 'thing-accent-text' : ''}`}>{game === 'know_me' ? (agreed ? c.gotIt : c.missedIt) : (agreed ? c.agreed : c.split)}</p><div className="grid grid-cols-2 gap-3 text-left">{round.answers.map((a) => <Pick key={a.display_name} label={a.display_name} value={option(a.answer_key)} />)}</div><button className={primary} disabled={pending} onClick={() => run(() => advanceChoiceEngine(snapshot.id, game))}>{c.next}</button><ErrorLine error={error} /></div>; }
...
function HotGame({ initial }: { initial: HotSnapshot }) {
  ...
  if (snapshot.gate) { const kitkat = snapshot.gate.target_level === 'kitkat'; const heading = kitkat ? c.kitkatWait : snapshot.gate.target_level === 'bold' ? c.wannaBold : c.wannaSpicy; const detail = kitkat ? c.kitkatMore : ''; const stay = snapshot.gate.target_level === 'bold' ? c.stayFlirty : snapshot.gate.target_level === 'spicy' ? c.stayBold : c.maybeNot; const go = snapshot.gate.target_level === 'bold' ? c.goBold : snapshot.gate.target_level === 'spicy' ? c.goSpicy : c.openKitkat; return <div className="space-y-6 py-10 text-center"><p className="font-heading text-4xl font-bold">{heading}</p>{detail && <p className="text-xl">{detail}</p>}{snapshot.gate.own_vote === null ? <div className="grid gap-3"><button className={secondary} disabled={pending} onClick={() => run(() => submitHotEscalation(snapshot.id, false))}>{stay}</button><button className={primary} disabled={pending} onClick={() => run(() => submitHotEscalation(snapshot.id, true))}>{go}</button></div> : <p role="status" className="text-muted">{c.waitingVote}</p>}<ErrorLine error={error} /></div>; }
  if (!round) return <div className={panel}>{c.busy}</div>;
  const prompt = locale === 'es' ? round.prompt_es : round.prompt_en; const optionA = locale === 'es' ? round.option_a_es : round.option_a_en; const optionB = locale === 'es' ? round.option_b_es : round.option_b_en;
  if (round.state === 'revealed') return <div className="space-y-6 text-center"><p className="thing-accent-text text-sm font-bold uppercase tracking-[.16em]">{c[snapshot.current_level]}</p>{snapshot.notice && <p className="font-heading text-2xl font-bold">{snapshot.notice === 'level_up' ? c.levelUp : c.stayingHere}</p>}<p className="text-2xl font-semibold">{prompt}</p><button className={primary} disabled={pending} onClick={() => run(() => advanceHot(snapshot.id))}>{c.next}</button><button className="min-h-11 text-sm underline" disabled={pending} onClick={() => run(() => completeHot(snapshot.id))}>{c.finishHere}</button><ErrorLine error={error} /></div>;
  return <div className="space-y-5"><p className="thing-accent-text text-sm font-bold uppercase tracking-[.16em]">{c[snapshot.current_level]}</p><AnswerRound hideLabel number={round.number} prompt={prompt} optionA={optionA} optionB={optionB} ownAnswer={round.own_answer} pending={pending} waiting={c.waitingPartner} onAnswer={(key) => run(() => submitHotRound(snapshot.id, round.id, key))} error={error} /><button className="min-h-11 w-full text-sm underline underline-offset-4" disabled={pending} onClick={() => run(() => skipHotPrompt(snapshot.id, round.id))}>{c.skip}</button></div>;
}
