...
function SameBrainGame({ initial }: { initial: SameBrainSnapshot }) {
  ...
  useEffect(() => {
    if (snapshot.state !== 'active' || (round?.state === 'answering' && !round.own_answer)) return;
    const refresh = async () => {
  ...
  if (snapshot.state === 'complete' && snapshot.result) {
    ...
    return <div className="space-y-6 text-center">
      <div className={`${panel} border-2`}><p className="text-sm font-bold uppercase tracking-[.18em]">{c.finalLine}</p><p className="text-7xl font-black">{result.matches} / {result.rounds}</p><h2 className="text-3xl font-bold">same brain.</h2><p>{Math.round(result.match_rate * 100)}% {c.matched}</p><p className="text-sm text-muted">{c.bestStreak}: {result.best_match_streak}</p></div>
      <Link className={`${primary} flex items-center justify-center`} href={`/thing/${snapshot.thing_id}/space`}>{c.seeSpace}</Link>
      <Link className={`${secondary} flex items-center justify-center`} href={`/thing/${snapshot.thing_id}`}>{c.backThing}</Link>
    </div>;
  }
  ...
  if (round.state === 'revealed') {
    ...
    return <div className="space-y-5 text-center">
      ...
      <div className={`${panel} border-2`}><p className="text-lg">{prompt}</p><p className="text-5xl font-black">{matched ? c.match : c.notThisTime}</p>{matched && <p className="text-xl font-semibold">{c.sameBrain}</p>}</div>
      ...
      <button className={primary} disabled={pending} onClick={() => run(() => advanceSameBrain(snapshot.id))}>{pending ? c.busy : c.next}</button><ErrorLine error={error} />
    </div>;
  }
  return <div className="space-y-5">
    ...
    <div className="grid gap-3"><button className="min-h-28 rounded-[24px] border-2 border-border bg-surface p-6 text-left text-xl font-bold disabled:opacity-60" disabled={pending || !!round.own_answer} onClick={() => run(() => submitSameBrainAnswer(snapshot.id, round.id, 'a'))}>{optionA}</button><button className="min-h-28 rounded-[24px] border-2 border-border bg-surface p-6 text-left text-xl font-bold disabled:opacity-60" disabled={pending || !!round.own_answer} onClick={() => run(() => submitSameBrainAnswer(snapshot.id, round.id, 'b'))}>{optionB}</button></div>
...
export function HangoutDetailScreen({ result, sameBrainResult }: { result: Result<HangoutSnapshot>; sameBrainResult?: Result<SameBrainSnapshot> }) {
  ...
  if (hangout.game_type === 'same_brain') return <Screen title={c.same_brain} backHref={`/thing/${hangout.thing_id}`}>
    {sameBrainResult?.ok ? <SameBrainGame initial={sameBrainResult.data} /> : <ErrorLine error={sameBrainResult?.error ?? 'connection_failed'} />}
  </Screen>;
  function add() {
    transition(async () => {
