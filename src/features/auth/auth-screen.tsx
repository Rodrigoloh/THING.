  ...
  return <div className="flex min-h-[calc(100dvh-7rem)] flex-col justify-center pb-8 sm:min-h-[calc(100dvh-10rem)]">
    ...
    <form className="space-y-4" onSubmit={(event) => { event.preventDefault(); void run(sentTo ? "verify" : "send"); }}>
      {sentTo ? <>
        ...
        <label className="block space-y-2"><span className="text-sm font-semibold">{t.emailCode}</span><input className="min-h-16 w-full rounded-2xl border border-border bg-surface px-5 text-center text-2xl tracking-[.2em] text-foreground" name="code" value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, "").slice(0, 10))} inputMode="numeric" autoComplete="one-time-code" pattern="[0-9]{6,10}" minLength={6} maxLength={10} required disabled={busy} autoFocus /></label>
