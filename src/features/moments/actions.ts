...
export async function loadMoments(thingId: string): Promise<Result<Moment[]>> {
  try {
    const client = await getSupabaseServerClient();
    const { data, error } = await client.from('moments').select('*').eq('thing_id', thingId).order('created_at', { ascending: false }).limit(100);
    if (error) return { ok: false, error: flowError(error) };
    const paths = (data ?? []).map((moment) => moment.storage_path);
    ...
    return { ok: true, data: (data ?? []).map((moment, index) => ({ ...moment, image_url: signed?.[index]?.signedUrl ?? '' })) };
