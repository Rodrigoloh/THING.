...
This implementation extends the existing `THING` main branch without replacing its Start / Join / Thing flow. It adds a fourth migration for short invitations and proposal-based Charm agreement, plus email/password authentication with the existing OTP flow retained as a fallback.
...
Do not deploy the UI before migration 004. Keep the existing Supabase URL/publishable-key settings; no service-role key or Realtime publication is required. Password signup/recovery requires the Auth configuration described in the README. Hosted migrations are not applied by tests.
...
The database suite applies all four migrations, creates minimal Supabase Auth/Storage metadata and JWT-role shims, and exercises the actual SQL functions/RLS. Two separate TCP connections are forced to wait on a locked row before releasing the race barrier. The unchanged original `things_rls.sql` runs after migration 002 and before later migrations because it describes the former direct-write API. Coverage includes short and legacy codes, collision retry, throttling, atomic join, expiry/revocation, outsider privacy, proposal replacement/acceptance, stale versions, concurrent proposal/acceptance, activation, guest and missing-profile rejection.
...
1. Apply migration 004, configure password email redirects/templates, then deploy the code.
